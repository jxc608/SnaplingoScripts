...
public class AutoTestRunner : TestsRunner 
{
    ...
    public override IEnumerator RunUIFixture(Type fixtureType, MethodInfo[] methods)
    {
		...
		LogManager.Log("TEST FIXTURE END: {0} ({1}s)\n{2}",
            fixtureType,
    ...
    void ReportText()
    {
		LogManager.Log(Delimeter , "\n");
		LogManager.Log("TESTS DURATION: " , FormatTime(DateTime.Now, timeStarted) , "\n");
		LogManager.Log("TESTS DURATION TOTALSECONDS: " , (DateTime.Now - timeStarted).TotalSeconds);
		LogManager.Log("\n" , Delimeter);
		LogManager.Log("ALL UI TESTS FINISHED");
		...
		LogManager.Log(output.ToString());
    }
