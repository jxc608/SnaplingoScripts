...
public class XcodeModifyGeneral
{
	...
	public static void OnPostprocessBuild(BuildTarget buildTarget, string path)
	{
		if (buildTarget == BuildTarget.iOS)
		{
			...
			rootDict.SetString("NSCameraUsageDescription", Application.productName + "想访问您的摄像机,允许吗?");
			rootDict.SetString("NSContactsUsageDescription", Application.productName + "想访问您的通讯录,允许吗?");
			rootDict.SetString("NSSpeechRecognitionUsageDescription", Application.productName + "需要在朗读环节记录您的读音，识别单词的发音是否正确。");
			rootDict.SetString("NSMicrophoneUsageDescription", Application.productName + "需要在朗读环节记录您的读音，识别单词的发音是否正确。");
