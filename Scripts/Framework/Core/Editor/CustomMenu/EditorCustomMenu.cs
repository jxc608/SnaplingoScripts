...
public class EditorCustomMenu : EditorWindow
{
	...
	[MenuItem ("自定义/工具/获得歌曲中所有中文字", false, 5)]
	static void GetAllChineseWords ()
	{
		EditorSongWordHandler handler = new EditorSongWordHandler ();
		handler.GetAndSave ();
	}
	...
	[MenuItem ("自定义/工具/获取所有关卡最高得分", false, 4)]
	static void GetAllLevelMaxScore ()
	{
		BeatmapParse.CalcAllTxtScore ();
	}

	[MenuItem ("自定义/工具/重置所有音频文件的文件名", false, 5)]
	static void CheckAllAudioFileNames ()
	{
		DirectoryInfo d = new DirectoryInfo (Application.dataPath + "/Resources/Audio/AudioWords");
		DirectoryInfo[] allDirs = d.GetDirectories ();

		foreach (DirectoryInfo dir in allDirs) {
			FileInfo[] infos = dir.GetFiles ();
			foreach (FileInfo f in infos) {
				File.Move (f.FullName, f.FullName.ToLower ().Replace (",", "").Replace ("?", "").Replace ("!", "").Replace ("'", "").Replace (" ", "_"));
			}
		}
	}
	...
	static void CutTexture ()
	{
		...
		cutPicture( filePath, savePath, filename );
		*/
		string filePath = "Assets/Artworks/test/xiaoxie.png";
	...
	static void cutPicture (string filepath, string savepath, string filename)
	{
		...
		for (int i = 0; i < tex.width; i++) {
			...
			if ((alpth[i] <= alpt || i == tex.width - 1) && Begin) {
