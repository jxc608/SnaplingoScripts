...
namespace Snaplingo.UI
{
	...
	public class PageManager : MonoBehaviour
	{
		...
		void PrintHistory()
		{
			LogManager.Log(MiscUtils.AssembleObjectsAsString<string>(_pageHistory.Keys, ","));
			var values = new List<string>();
			for (int i = 0; i < _pageHistory.Values.Count; ++i)
			{
				values.Add(_pageHistory.Values[i]._item1);
			}
			LogManager.Log(MiscUtils.AssembleObjectsAsString<string>(values, ","));
		}
		...
		Page CreatePage(string pageType, List<string> nodes)
		{
			...
			{
				LogManager.LogError("Error! Cannot find page: " , pageType);
			}
			return page;
		}
