...
//lyricName = SongConfig.Instance.m_items[levelData.songID]["lyric"];

public class SongConfig : IConfig
{
	public static SongConfig Instance { get { return _instance; } }
	static SongConfig _instance;
	public SongConfig()
	{
		_instance = this;
	}
	...
	public void Fill(string jsonstr)
	{
		...
		for (int i = 0; i < keys.Count; i++)
		{
			try
			{
				...
				string s = value.TryGetString("songID");
				int id = int.Parse(s);
				...
				items["lyric"] = value.TryGetString("lyric");
				m_items[id] = items;
			}
			catch (Exception e)
			{
				LogManager.LogError("Song Config row " , i , " parse Error! " , e);
			}
		}
	}
	...
	public int GetSongOffsetBySongIDAndLevelDiffculty(int songID, int LevelDifficulty)
	{
		...
		if (script.Length != 2)
		{
			LogManager.LogError("songConfig offset error! songID:" , songID);
			return 0;
		}
		else
		{
